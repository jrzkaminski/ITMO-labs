...
// Measure time of non-parallel calculations
...
// Measure time of parallel calculations for each order
...
int main(int argc, char* argv[]) {
  ...
  // measuring time of ijk for non-parallell process 
  cout << "Measure ijk loops" << endl;
  auto sthread_time = measure_time(A, B, C, N, "ijk");
  cout << "Single thread time " << sthread_time << endl;
  //measuring time of ijk parallel proccess
  for (int num_thr = 1; num_thr <= 10; num_thr++) {
    ...
    cout << "Efficiency of " << num_thr << " thread(s) = " << sthread_time / parallel_time << endl;
  }
  //zeroing result matrix
  zero_init_matrix(C, N);
  // same for the jki
  cout << "Measure jki loops" << endl;
  sthread_time = measure_time(A, B, C, N, "jki");
  cout << "Single thread time " << sthread_time << endl;
  for (int num_thr = 1; num_thr <= 10; num_thr++) {
    zero_init_matrix(C, N);
  ...
  sthread_time = measure_time(A, B, C, N, "ikj");
  cout << "Single thread time " << sthread_time << endl;
  for (int num_thr = 1; num_thr <= 10; num_thr++) {
    zero_init_matrix(C, N);
